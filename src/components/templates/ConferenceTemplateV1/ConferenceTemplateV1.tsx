 ...
 *
 * Features:
 * - Scroll-triggered section animations (subtle by default)
 * - Staggered section reveals for polished appearance
 * - Reduced motion preference support
 */
export function ConferenceTemplateV1({
  config,
  assets,
  eventId,
  eventSlug,
  ...
  const mobileNavItems = sections
    .filter((s) => s.enabled && shouldShowInNav(s, "conference"))
    .filter((s) => s.type !== "rsvp" || Boolean(eventSlug))
    .map((s) => ({
      id: s.type,
      label: resolveNavLabel(s, "conference"),
      href: `#${s.type}`,
      isCta: s.type === "rsvp",
    }));
