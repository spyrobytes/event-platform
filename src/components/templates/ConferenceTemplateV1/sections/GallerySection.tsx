 ...
 * Professional style with keyboard-accessible lightbox and multiple display modes.
 */
export function GallerySection({ data, assets, primaryColor }: GallerySectionProps) {
  ...
  // Render grid layout - Professional conference style
  const renderGrid = () => (
      ...
      {galleryItems.length > 3 && (
        <>
          <button
            ...
            className="absolute -left-4 top-1/2 z-10 -translate-y-1/2 rounded-lg bg-white/90 p-2 shadow-lg transition-all hover:bg-white"
          ...
          <button
            ...
            className="absolute -right-4 top-1/2 z-10 -translate-y-1/2 rounded-lg bg-white/90 p-2 shadow-lg transition-all hover:bg-white"
    ...
    >
      <div className="relative aspect-[16/9] overflow-hidden rounded-xl bg-muted">
        ...
        {galleryItems.length > 1 && (
          <>
            <button
              type="button"
              onClick={goToPrev}
              className="absolute left-4 top-1/2 -translate-y-1/2 rounded-lg bg-white/20 p-3 text-white backdrop-blur-sm transition-all hover:bg-white/30"
            ...
            <button
              type="button"
              onClick={goToNext}
              className="absolute right-4 top-1/2 -translate-y-1/2 rounded-lg bg-white/20 p-3 text-white backdrop-blur-sm transition-all hover:bg-white/30"
        ...
        <button
          ...
          className="absolute right-4 top-4 rounded-lg bg-white/20 p-2 text-white backdrop-blur-sm transition-all hover:bg-white/30"
        ...
        {autoPlay && isPaused && (
          <div className="absolute left-4 top-4 rounded-lg bg-white/20 px-3 py-1 text-xs text-white backdrop-blur-sm">
            Paused
          </div>
        )}
      </div>
          ...
          {galleryItems.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                index === currentSlide ? "w-6" : "w-2 bg-muted-foreground/30 hover:bg-muted-foreground/50"
              }`}
              style={{
    ...
    <SectionWrapper ariaLabel="Photo gallery" className="bg-muted/30">
      ...
      {lightboxIndex !== null && currentItem && (
        ...
        >
          ...
          {autoPlay && (
            <div className="absolute left-4 top-4 z-10 flex items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 text-sm text-white">
              {isPaused ? (
                <>
